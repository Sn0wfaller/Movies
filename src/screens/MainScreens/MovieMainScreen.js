...
const MovieMainScreen = ({ navigation }) => {
  ...
  const getData = () => {
    ...
    MovieDataProvider.getAllMoviesShortInfo().then((movies) => {
      const mappedMovies = movies.data.map((movie) => ({
        key: movie.id,
        ...movie,
      }));
  ...
  const handleDelete = (id) => {
    MovieDataProvider.deleteMovie(id).then(getData);
  };
  ...
  return (
    <Card>
      ...
      />
      <Card.Divider />
      <FlatList style={styles.list} data={movies} renderItem={renderListItem} />
    </Card>
  );
};
